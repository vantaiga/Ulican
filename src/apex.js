...
// Fixed: provider singletons at module load (no OOM)
// Fixed: cleanHex strips Railway chars from private key
// Fixed: continuous counter, never stops
// No logs about wallet/executor — key is in env var, that is sufficient
...
async function execute(slot) {
  ...
  try {
    ...
    const cd   = IFACE.encodeFunctionData('flashLoan', [
      contract,
      [USDC[cid] || USDC[137]],
      [BigInt(Math.floor(Math.min(profit*200, P.flash)))],
...
poll()
console.log('[APEX] ULICAN Model 1 online')
